...
namespace nezumi
{
    ...
    static bool window_event(SDL_Event& event) 
    {
        switch (event.window.event) 
        {
            ...
            {
                g_window_x = event.window.data1;
                g_window_y = event.window.data2;
    ...
    {
        SDL_SetWindowTitle(reinterpret_cast<SDL_Window*>(g_window_handle), title);
    }

    void window_center()
    {
        SDL_SetWindowPosition(reinterpret_cast<SDL_Window*>(g_window_handle), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED);
    }

    void window_set_size(int width, int height)
    {
        SDL_SetWindowSize(reinterpret_cast<SDL_Window*>(g_window_handle), width, height);
    }

    void window_set_position(int x, int y) 
    {
        SDL_SetWindowPosition(reinterpret_cast<SDL_Window*>(g_window_handle), x, y);
    }

    void window_set_visible(bool visible)
    {
        (visible) ? SDL_ShowWindow(reinterpret_cast<SDL_Window*>(g_window_handle)) : SDL_HideWindow(reinterpret_cast<SDL_Window*>(g_window_handle));
    }
    ...
    void window_set_fullscreen(bool fullscreen, bool borderless)
    {
        ...
        if (fullscreen) {
            if (borderless) 
                ret = SDL_SetWindowFullscreen(reinterpret_cast<SDL_Window*>(g_window_handle), SDL_WINDOW_FULLSCREEN_DESKTOP);
            else
                ret = SDL_SetWindowFullscreen(reinterpret_cast<SDL_Window*>(g_window_handle), SDL_WINDOW_FULLSCREEN);
        }
        else ret = SDL_SetWindowFullscreen(reinterpret_cast<SDL_Window*>(g_window_handle), 0);
